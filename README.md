...
### 12) Changed application title and description metadata
- **Issue:** The app still used default Next.js scaffold metadata (`Create Next App` / `Generated by create next app`) in the browser tab and page head.
- **Fix:** Updated `metadata` in `app/layout.tsx` to use project-specific values (`StackShop` and a product-focused description).
- **Why:** Ensures consistent branding and cleaner global metadata without duplicating title/description logic across pages.
